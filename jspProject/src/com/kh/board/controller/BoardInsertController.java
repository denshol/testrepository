...
public class BoardInsertController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		 * 
		 * form에서 multipart/form-date 형식으로 전송했기때문에 기존 request에 parameter영역에서
		...
		*/
		...
		//request를 multipart 객체로 이관시켜숴 다뤄야 한다.
		...
		if(ServletFileUpload.isMultipartContent(request)) {
			 ...
			 * WebContent 에 board_files 폴더 경로까지는 잡아주어야 한다 폴더에 저장될것이기 때문에.
			 * 
			 * 
			 * */
...
//			System.out.println(savePath);
			 ...
			 * 한글/특수문자가 포함된 파일명경우는 업로드 되는 서버에 따라 오류를 발생시킬 여지가 있기떄문.
			 * 
			 * 기본적으로 파일명을 변경해주는 객체를 제공한다
			 * DefaultFileRenamepolicy 객체
			 * 내부적으로 rename()이라는 메소드가 실행되며 파일명 수정을 해준다.,
			 * 
			 * 기본적으로 제공된 객체는 파일명 수정을 파일명 뒤에 숫자로 카운팅하는 형식으로만 진행해준다.
			...
			//첨부파일이 있을 경우  원본명 반환 없는경우 null 반환
			if(multiRequest.getOriginalFileName("upfile")!=null ) {
				//조회가 된경우 (첨부파일이 있다)
				at = new AttachMent();
				at.setOriginName(multiRequest.getOriginalFileName("upfile")); //원본 명
				at.setChangeName(multiRequest.getFilesystemName("upfile"));   //수정 명(실제 서버에 업로드된 파일 명)
			...
			if(result>0) {
				...
				if(at!=null) {//넘어온 파일이 있어서 객체가 생성했다면,
