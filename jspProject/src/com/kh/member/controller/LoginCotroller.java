...
//		new MemberService().loginMember(userId,userPwd);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * -request : 서버로 요청할 때의정보들이 담겨있다 (요청시 전달값, 요청 전송 방식 등등)
		 * -response : 요청에 대해 응답에 필요한 객체
		 * 
		 * GET / POST 방식 차이
		 * -GET : 사용자가 입력한 값이 url에 노출되고 데이터의 길이제한이 있으며 즐겨찾기를 할수있다.
...
//		System.out.println("조회된 Member : "+m);
		...
		/*
		 * 응답페이지에 전달할 값이 있다면 값을 어딘가에 담아서 전달해야한다. (이떄 담아줄수있는 내장객체 4가지)
		 * -servlet scope
		 ...
		 * 3)request : request 에 담은 데이터 해당 request를 포워딩한 응답 jsp에서만 꺼내쓸수있다.
		...
		}else {  //로그인 성송 - index 페이지로 돌아가기
			//로그인한 회원의 정보를 계속 가지고 다닐것이기 떄문에 session에 담아준다  
...
//			response.sendRedirect("/jsp"); // localhost:8888/jsp   context 루트로 기존페이지
